...
function sliceUploadFile(params, callback) {
    ...
    var SleepTime = params.SleepTime;
    var Key = params.Key;
    ...
    ep.on('upload_slice_complete', function (UploadData) {
        console.log('==============> upload_slice_complete');
    ...
    ep.on('get_upload_data_finish', function (UploadData) {
        console.log('==============> get_upload_data_finish');
        // 处理 UploadId 缓存
    ...
    ep.on('get_file_size_finish', function () {
        console.log('==============> get_file_size_finish');
...
//     });
// }
async function uploadSliceList(params, cb) {
    var self = this;
    var SleepTime = params.SleepTime;
    var TaskId = params.TaskId;
    var Bucket = params.Bucket;
    ...
    try{
        for(let SliceItem of needUploadSlices){
            if (!self._isRunningTask(TaskId)) return;
            var PartNumber = SliceItem['PartNumber'];
            var currentSize = Math.min(FileSize, SliceItem['PartNumber'] * SliceSize) - (SliceItem['PartNumber'] - 1) * SliceSize;
            var preAddSize = 0;
    ...
    //     });
    // }, function (err) {
    //     console.log('====self._isRunningTask(TaskId) ====>', self._isRunningTask(TaskId));
    //     if (!self._isRunningTask(TaskId)) return;
    //     if (err) return cb(err);
    //     cb(null, {
    //         UploadId: UploadData.UploadId,
